...
<div role="main" class="main">
			...
			<section class="section section-parallax parallax custom-parallax-medium-padding m-none" data-plugin-parallax data-plugin-options="{'speed': 1.5}" data-image-src="{{('img/' . $titlesPage->images)}}">
				<div class="container">
					<div class="row">
						<div class="col-md-12">
					<h1 class="text-uppercase text-color-light font-weight-bold center m-none">{{$titlesPage->
titles}}</h1>
						</div>
					</div>
				</div>
			...
			<section class="section section-no-border section-tertiary custom-section-spacement-1 m-none">
				<div class="container">
					<div class="row center">
						<div class="col-md-12">
							<h2 class="text-uppercase text-color-dark font-weight-bold">{{$titlesPage->title}}</h2>
							<p class="text-md text-color-dark mb-xlg">{{$titlesPage->sub_title}}</p>
						</div>
					</div>
					<div class="row">
					@foreach($services as $service)
						<div class="col-md-4 col-sm-6">
						    <a href="{{ route('serviceShow',['id'=>$service->custom_url]) }}" >
							<article class="custom-post-style-1">
								<img style="height:194px;width:360px;" src="{{('imgServices/' . $service->images)}}" alt class="img-responsive mb-xlg" />
								<h3 class="text-color-dark font-weight-bold mb-md">{{$service->titles}}</h3>
								<p class="custom-text-color-1">{!! str_limit( $service->content,200) !!}</p>
